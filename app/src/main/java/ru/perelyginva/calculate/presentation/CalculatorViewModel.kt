...
class CalculatorViewModel : ViewModel() {
    ...
    private fun negate() {
        val resultFromLastOperator = state.displayNumber != "" && state.number1 == ""
        val zeroShouldNotBeNegation = state.displayNumber == ""
                && state.displayNumber.toDoubleOrNull() == 0.0
        val negateFirstNumber = state.number2 == "" && state.number1 != ""

        if (resultFromLastOperator) {
            val negatedValue = negativeValue(state.displayNumber)
            state = state.copy(
                displayNumber = negatedValue,

                )
        } else if (zeroShouldNotBeNegation) {
            return

        } else if (negateFirstNumber) {
            val negatedValue = negativeValue(state.number1)
            state = state.copy(
                number1 = negatedValue,
                displayNumber = negatedValue
            )
        } else {
            val negatedValue = negativeValue(state.number2)
            state = state.copy(
    ...
    private fun negativeValue(number: String): String {
    ...
    private fun calculate() {
        ...
        if (operand1 != null && operand2 != null) {
            ...
            state = state.copy(
                displayNumber = dropZeroAfterDecimal(result),
                currentEquation = "",
                operation = null,
                number1 = "",
                number2 = "",
    ...
    private fun writeOperator(operation: MathOperation) {
        ...
        //теперь мы указываем, что если оператор не равен null
        // и второго числа нет, делай следующее
        val shouldOverwriteOperator = state.operation != null && state.number2 == ""
        //если первое число равно null и на экране есть число, то делай
    ...
    private fun writeFullEquation(result: Double): String {
        val resultString = dropZeroAfterDecimal(result)
        val equation = if (state.operation != null) {
            state.number1 +
                    "" + state.operation!!.symbol +
                    "" + state.number2 + " = " + resultString
        } else {
            state.number1
        }
        return equation
    ...
    private fun dropZeroAfterDecimal(number: Double): String {
    ...
    private fun isWholeNumber(number: Double): Boolean {
        //если делиться на 1, то это целое число
        if (number % 1 == 0.0) {
            return true
        }
        return false
